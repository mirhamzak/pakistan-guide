import { AppData, Bookmark, SearchResult } from '@/types';
...
class StorageService {
  ...
  private async saveToSQLite(data: AppData): Promise<void> {
    if (!this.db) return;
  ...
  async getHomeScreenSearchSuggestions(query: string, limit: number = 10): Promise<string[]> {
    if (!this.db || query.length < 2) return [];
  ...
  async searchHomeScreenContent(query: string, homeScreenData: any): Promise<any[]> {
    try {
      ...
      homeScreenData.contentSections.forEach((section: any, index: number) => {
      ...
      homeScreenData.citizenFeatures.forEach((category: any, categoryIndex: number) => {
  ...
  async searchContent(query: string): Promise<SearchResult[]> {
    ...
    try {
      ...
      results.push(...generalResults.map((row: any) => ({
        id: row.id,
        title: row.title,
        content: row.content,
      ...
      results.push(...emergencyResults.map((row: any) => ({
        id: row.id,
        title: row.title,
        content: row.content,
      ...
      results.push(...travelResults.map((row: any) => ({
        id: row.id,
        title: row.title,
        content: row.content,
      ...
      results.push(...languageResults.map((row: any) => ({
        id: row.id,
        title: row.title,
        content: row.content,
      ...
      results.push(...lawResults.map((row: any) => ({
        id: row.id,
        title: row.title,
        content: row.content,
        type: 'law' as const,
      ...
      results.push(...culturalResults.map((row: any) => ({
        id: row.id,
        title: row.title,
        content: row.content,
  ...
  async getSearchHistory(): Promise<{ query: string; timestamp: string }[]> {
    try {
